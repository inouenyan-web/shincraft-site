...
### 0. ShinCRAFT_photoroom → 01_投稿待ち 自動コピー（毎回必ず実行）
- Drive MCP で `ShinCRAFT_photoroom`（ID: `1vpJbqdCtwvvNIPDO09BAiml7_Q_4qs6c`）のファイルを列挙。
- ファイルがあれば、Drive MCP の `search_files` で `01_投稿待ち` 内に同名ファイルが **既にあるかチェック**。
  - 未コピーのものだけ `copy_file`（`parentId: 17BVeGqN2A7Kj_ppMxGXz-Nejim7UQj0j`）でコピー。
  - 既にあるものはスキップ（重複防止）。
- コピーしたファイル名を一言報告してから次のステップへ進む。
- ※ Drive MCPに削除機能がないため元ファイルは `ShinCRAFT_photoroom` に残る（許容）。
