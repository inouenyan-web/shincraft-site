...
| 06-25 | **「Cowork」タブと「Code」タブは表示機能が別物**。複数セッションを画面に分割表示（最大4ペイン）できるのは**Codeタブ（Claude Code）だけ**。**Coworkタブは会話1つ＋成果物プレビューの表示で、複数セッション会話を画面に並べられない**（複数タスクの並行"実行"は裏で可能だが、同時に"目視"はできない＝非同期タスク前提の設計。マルチウィンドウ/デュアルモニターは要望中の未実装）。Web検索でCode側の情報をCoworkに当てはめて誤説明し、井上さんにスクショで指摘された。**製品UIの仕様は実機/スクショで確認してから答える。CodeとCoworkを混同しない。** |
| 06-25 | **ハード購入相談は「現有資産の実測」と「ボトルネックの特定」が先**。井上さんの現用機 LAVIE N1455/HAL は **16GBオンボード半田付け・増設不可・Win11 Home**（コワークはPro必須）。「コワーク2台仕様でMacBook Air 24万」案は、①Coworkは1台で並行実行可、②並行"表示"は元々不可（上記）、③据置運用なら携帯性は無駄、で不要と判明。買うなら「持ち運べるMac」でなく将来「Win11 Pro・32GB」。当面は外部モニター(1〜2万)で十分。**端末を増やす前に、何がボトルネックかを数字で確定する。** |
