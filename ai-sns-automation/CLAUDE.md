...
2. Claude Code が **処理しきれない時だけ**、次の優先順位で委譲する：
   ...
   | 優先 | 委譲先 | 役割（得意なこと） |
   |---|---|---|
   | 1 | **コワーク** | Anthropic Claude Cowork（デスクトップのエージェント型ワークスペース）。Chrome版と連動・スケジュール実行可。※人間ではない |
...
5. **委譲先に振る前に、その相手が物理的に実行できる操作かを確認する。**
   できない作業を振らない（例：Claude Code環境変数の更新はChrome版では不可）。
   同じコピペ手順を繰り返し投げ直さない。1回で通らなければ即アプローチを変える。
   井上さんに手作業を頼むのは、認証承認・公開デプロイ確定・環境変数設定・投稿承認など
   **本人にしかできない操作だけ**（詳細はリポジトリ全体の `CLAUDE.md` を参照）。

「Claude Code が処理しきれない」の判断基準は `DELEGATION.md` を参照。

### 受信箱ハンドオフ（コワーク・チャットへの自動委譲の仕組み）

内部エージェント（社内部署）がコワークやチャットに仕事を渡す時は、
**井上さんを経由せず** 以下のプロトコルで自動的に受け渡す。

```
内部エージェント（sns-text-generator等）
  → 成果を Claude Code（オーケストレーター）に返す
  → Claude Code が drive_inbox.mjs でタスクを受信箱Docに書き込む
  → コワーク：起動時に受信箱を読んで pending タスクを実行 → 結果を受信箱に書き戻す
  → チャット：セッション起動時に CHAT_BRIEFING_PROMPT.md の内容を読み受信箱を確認
  → Claude Code：次のセッションで受信箱を読み結果を回収
```

**受信箱 Doc**: `1alE6ds2j-iGG-n_vG1wslVuZfo19q-z66ibJiLvd2_g`（AI親フォルダ内）

**タスクを書く（Claude Codeが実行）**:
```javascript
import { writeTask } from './scripts/lib/drive_inbox.mjs';
await writeTask({
  from: 'sns-text-generator',
  to: 'cowork',           // "cowork" | "chat" | "chrome"
  priority: 'high',
  type: 'browser_operation',
  title: 'BASE申請フォーム入力',
  instructions: '1. URLを開く\n2. フォームに入力\n3. 送信直前で止める',
  expectedOutput: 'フォーム入力完了後のスクリーンショット',
});
```

**コワーク起動手順**: `COWORK_STARTUP_PROMPT.md` を参照。
**チャット起動手順**: `CHAT_BRIEFING_PROMPT.md` の貼り付け用プロンプトを使う。
...
| AI報告受信箱 Doc | `1alE6ds2j-iGG-n_vG1wslVuZfo19q-z66ibJiLvd2_g` |
| Cowork起動手順_最新 Doc | `1RE3PcG0BwER5fWFUXJxH2akKAVdM-Q_SFVZFRp1yiIw` |
