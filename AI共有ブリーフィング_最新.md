...
### 指揮系統（コードが陣頭指揮・外部AIも傘下）

```
井上さん（承認だけ）
  └─ コード（Claude Code）＝陣頭指揮。タスク分解と指示書作成まで全部やる
       ├─ GitHub Actions ……… 24時間の定期実行（報告会・チェック・ミラー）
       ├─ コワーク ……………… ローカル/ブラウザ作業の実行部隊（スケジュール可）
