...
### 📨 2026-07-01 20:55 Instagram DM チェック（Meta Business Suite）
...
<!-- GMAIL_INBOX:START -->
### 📧 2026-07-01 20:55 Gmail 新着チェック（自動 / shincraft2023）

直近24h未読 7件 / うち受注関連の可能性 3件

**🧾 受注関連の可能性（要対応・チェックでクローズ）:**
- [ ] "moncheri BtoB 卸担当 横尾" <sales@btob-monch… ｜ 【初回発注15%OFF】犬服ブランドmoncheri 卸販売 /Interpet2026大阪にご来場いただきありがとうご…
- [ ] "ラクスル" <mag@raksul.com> ｜ ラクスル会員様限定◆チラシ印刷やその他複数商品が今なら50%OFF！
- [ ] send-only@iv.kingoftime.jp ｜ [KING OF TIME]2026年06月分 請求書発行のご連絡（INV02042610）
<!-- GMAIL_INBOX:END -->

<!-- CHATWORK_INBOX:START -->
### 💬 2026-07-01 20:55 Chatwork 新着チェック（既読ルーム精査）

既読ルーム 1件を走査 / 途中・要対応 0件 / 受注関連 1件

**🧾 受注関連（会話中に受注の話あり）:**
- [ ] [マイチャット] Boost WORKS: /inbox 受注チェック（2026/6/30 12:34:48 JST） [警告] 要対応（未返信） ・[IG DM] [DM] y.s：PayPay支払いの相談「いま残高ないの…

**📨 未読あり（消費しないので手動で確認してください）:**
- 長井 祥和：未読 1件
<!-- CHATWORK_INBOX:END -->

<!-- IG_DM_INBOX:START -->
### 📩 2026-07-01 20:55 Instagram DM 新着チェック（会話精査）

会話 0件を走査 / 途中・要対応 0件 / 受注関連 0件
...
<!-- IG_DM_MANUAL:START -->
### 📩 2026-07-01 06:10 JST Instagram DM 受注・要対応（Cowork目視 / Meta Business Suiteより）

- [ ] 🔴 [DM] y.s：iQOSイルマワン両面加工（本体カラー黒）8500円(送料込)。**PayPay送金完了**（送金者名「ぐろりあ」・火21:51）→ 入金確認＋お礼＋制作開始の返信が**未送信＝最優先で対応**
- [ ] 🟡 [DM] 高橋じゅん：名入れスタンド（漢字「千鶴」・大サイズ7660円）。PayPay入金済→発送→本日0:29「本日受け取りました、ありがとうございます」着＝納品完了。一言お礼返信を推奨（受注完了・今回新規ピックアップ）
- [ ] 🟡 [DM] やっちゃん：「iQOSありがとうございました。インスタあげてもらっても大丈夫ですよ」→ 納品完了・掲載許可取得済み。一言お礼返信を推奨（投稿OK）
- [ ] 🟡 [DM] Camaro&N-ONE&Fu~ma♡シュナ&ポメーズ♡：当方「デザインは何ていれますか？」送信済み（アウトバウンド）→ デザイン内容の返信待ち（名入れ案件・フォロー）
- [ ] 🟡 [DM] 犬の訪問整体 健犬@bridge：当方が制作サンプルを提示済み（「他のお客さんで作った商品 こんな感じです」）→ 相手の返信待ち（制作案件・フォロー）
- [ ] 🟡 [DM] Dogs&Cats Special party：当方「すぐ振込いたします」送信済み（アウトバウンド・支払い関連）→ 相手の返信待ち（フォロー）
- [ ] 🟡 [DM] DogRun Cafe Pic：「ありがとうございます 楽しみにしております」＝納品待ち（お礼＋期待）→ 納品進行を確認

（お礼のみ/いいね/ストーリーズメンション/雑談はタスク化対象外：高橋じゅん最後の発言は受領お礼だが受注完了案件のため掲載）
※今回の直近会話ウィンドウ7件を精査。前回ログのディープさん（タイニープードル）・Boost Works等は今回ウィンドウに非表示（旧ログ扱い／必要時に受信箱で要確認）。
<!-- IG_DM_MANUAL:END -->
